...
def test_read_client(test_client, client, token):
    ...
    rv = test_client.get("/configure_client/client_id", headers=headers)
    resp = json.loads(rv.data)
    assert rv.status_code == 200
    assert resp["client_id"] == client.client_id
    assert resp["client_name"] == "Authlib"
    assert (
        resp["registration_client_uri"] == "http://localhost/configure_client/client_id"
    )
    assert resp["registration_access_token"] == token.access_token


def test_read_access_denied(test_client):
    rv = test_client.get("/configure_client/client_id")
    resp = json.loads(rv.data)
...
def test_update_client(test_client, client, token):
    ...
    rv = test_client.put("/configure_client/client_id", json=body, headers=headers)
    resp = json.loads(rv.data)
    assert rv.status_code == 200
    assert resp["client_id"] == client.client_id
...
def test_update_access_denied(test_client):
    rv = test_client.put("/configure_client/client_id", json={})
    resp = json.loads(rv.data)
    assert rv.status_code == 400
    assert resp["error"] == "access_denied"

    headers = {"Authorization": "bearer invalid_token"}
    rv = test_client.put("/configure_client/client_id", json={}, headers=headers)
    resp = json.loads(rv.data)
    assert rv.status_code == 400
    assert resp["error"] == "access_denied"
...
def test_delete_client(test_client, client, token):
    assert client.client_name == "Authlib"
    headers = {"Authorization": f"bearer {token.access_token}"}
    rv = test_client.delete("/configure_client/client_id", headers=headers)
    assert rv.status_code == 204
    assert not rv.data


def test_delete_access_denied(test_client):
    rv = test_client.delete("/configure_client/client_id")
    resp = json.loads(rv.data)
    assert rv.status_code == 400
    assert resp["error"] == "access_denied"
